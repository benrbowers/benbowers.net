...
function electo_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'electo-large-thumb', 830 );
	add_image_size( 'electo-medium-thumb', 550, 400, true );
	add_image_size( 'electo-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'electo' )
	) );
...
add_action( 'after_setup_theme', 'electo_setup' );
...
add_action('electo_before_site', 'electo_preloader');
...
function electo_grid_post_meta(){
...
function electo_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'electo') . '</a>';
}
add_filter('excerpt_more', 'electo_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function electo_scripts() {
	...
	wp_enqueue_script( 'electo-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'electo-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'electo-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
	
}
add_action( 'wp_enqueue_scripts', 'electo_scripts' );

/**
 * Enqueue Bootstrap
 */
function electo_enqueue_bootstrap() {
	wp_enqueue_style( 'electo-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'electo_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'electo_widgets_init' );
function electo_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'electo' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'electo' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'electo' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'electo' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function electo_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'electo' ), get_the_title() ),
...
function electo_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'electo').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "electo");
        echo '"<em>';
...
function electo_recommended_plugin() {
...
add_action( 'tgmpa_register', 'electo_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'electo_check_for_update' );
function electo_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 30,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Electo_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Electo',
	'twitter' => 'https://twitter.com',
	'facebook' => 'https://www.facebook.com/',
	'website' => 'http://suavethemes.com',
]);

function electo_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'electo_upsell_script' );